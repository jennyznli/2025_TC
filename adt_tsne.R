...
tsne_primary <- read.csv(file.path("data", "adt_tsne_primary_coords.csv")) 
...
tsne_primary_clean <- tsne_primary %>% select(-all_of(overlap_cols))
tsne_all_clean     <- tsne_all %>% select(-all_of(overlap_cols))

tsne_primary <- inner_join(tsne_primary_clean, ss, by = "IDAT")
tsne_all     <- inner_join(tsne_all_clean, ss, by = "IDAT")

tsne_driver <- tsne_primary %>% 
...
# ========================
data_subsets <- list(
  list(name = "primary", data = tsne_primary)
