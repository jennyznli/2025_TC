...
ss_primary$CC_Cluster <- as.factor(ss_primary$CC_Cluster)
...
se_ok <- (checkLevels(assay(se), colData(se)$Sex) &
              checkLevels(assay(se), colData(se)$CC_Cluster))
...
colData(se)$CC_Cluster <- relevel(factor(colData(se)$CC_Cluster), "LEUKO")

smry <- DML(se, ~CC_Cluster + Sex,
...
saveRDS(res_inv, file = file.path(DATA_DIR, "ped_dm_cluster_leuko.rds"))
