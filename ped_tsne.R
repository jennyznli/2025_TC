# ============================================================
#   This script plots tSNE visualizations for the pediatric cohort.
# ============================================================
source("../config.R")
...
# ========================
# MERGE MIR200C AND RNA EXPRESSION DATA 
# ========================
...
write.csv(ss, file.path("ss", PED_META))
...
create_confidence_plot <- function(data, color_var, color_values, shape_var, 
  ...
  if (nrow(no_confidence) > 0) {
    p <- p + geom_point(data = no_confidence, aes(color = !!sym(color_var), alpha = 0.7), size = 2, shape = 1)
  }
  if (nrow(has_confidence) > 0) {
    p <- p + geom_point(data = has_confidence, aes(color = !!sym(color_var), shape = !!sym(shape_var), 
                                                   alpha = !!sym(alpha_var)), size = 2) +
      scale_shape_manual(values = shape_values) +
      scale_alpha_continuous(range = c(0.4, 1))
  }
  p <- p + scale_color_manual(values = color_values)
...
# ========================
lymph_nodes <- tsne_all_12 %>% filter(Lymph_Node == "T") 
...
for (i in 1:nrow(lymph_nodes)) {
  ...
  if (nrow(primary) > 0) {
    ln_row <- ln %>% mutate(pair_id = paste0("Pair ", pair_id), sample_type = "Lymph Node")
    primary_row <- primary %>% mutate(pair_id = paste0("Pair ", pair_id), sample_type = "Primary Tumor")
...
pairs_data <- pairs_data %>%
  mutate(Pair_ID = ifelse(sample_type == "Lymph Node",Paired_Primary, Sample_ID))
...
# remove duplicate primary tumors for plotting
pairs_data <- pairs_data %>% distinct(Sample_ID, .keep_all = TRUE)

# color mapping
n_pairs <- 13
pair_color_mapping <- setNames(pair_colors[1:n_pairs], unique(pairs_data$Pair_ID))

tsne_all_12 <- tsne_all_12 %>% mutate(is_paired = Sample_ID %in% pairs_data$Sample_ID)

p_paired <- ggplot() +
  geom_point(data = tsne_all_12 %>% filter(!is_paired), aes(x = tSNE1, y = tSNE2),
             color = "grey80", alpha = 0.4, size = 1.5) +
  geom_segment(data = connections, aes(x = x1, y = y1, xend = x2, yend = y2, color = Pair_ID),
               linetype = "dashed", linewidth = 0.7) +
  geom_point(data = pairs_data, aes(x = tSNE1, y = tSNE2, color = Pair_ID,
                 shape = sample_type, size = sample_type), alpha = 0.8) +
  scale_color_manual(values = pair_color_mapping, name = "Sample ID") +
  scale_shape_manual(values = c("Lymph Node" = 17, "Primary Tumor" = 1), name = "Sample Type") +
  scale_size_manual(values = c("Lymph Node" = 3, "Primary Tumor" = 3.5), name = "Sample Type") +
